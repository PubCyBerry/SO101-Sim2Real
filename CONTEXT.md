...
- **커밋**: `8a97780`(v3 본체) → `6a1be70`(20초 단축) → `55a6294`(장애물 회피·동적 순서·DRAG) → `07cbd7a`(release 재설계·spawn 제약·drop 복구). 영상(구버전 정리 후 2종): `docs/so101_pick_place_single-step-0.mp4`(1env DR), `docs/so101_pick_place_multi4-step-0.mp4`(4env DR 전체 부감, 16/16).
...
- **5차 (커밋 `687c00c`→`ac50657`)**: ① RETREAT 내리꽂기 — z-ramp 시작점 고정 가정(place_height)이 실제 TCP(safe_z)보다 낮아 release 직후 그릇으로 내려갔다 올라옴 → 실측 TCP 시작. ② 슬램덩크 — TRANSPORT 진입 시 ik_reach 의 pitch 단번 완화(−90→−40°)가 slew 풀속도 재배향 유발 → **xy ramp frac 에 pitch·wrist roll(release 90°) 동시 점진 보간** (위치만 ramp 말고 자세도 ramp). 그릇 위 회전 단계는 정착 게이트로 축소. **최종: 1env DR 4/4(슬램덩크 소멸 영상 확인), 4env DR 16/16 (100%).**
