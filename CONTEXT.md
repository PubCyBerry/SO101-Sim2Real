...
- **결과(1큐브 DR-on 20ep, 동일 seed)**: random_fk(개선환경) **85%** vs deterministic v6 **90%**(18/20, 실패=reach 경계 spawn). 버전사: v1 90%→v2 85%(속도·tilt잠금)→v3 90%(음수tilt 제거)→v4 80%(깊이 -8mm 회귀)→v5b 80%(-4mm+침투필터 -3mm 함정)→**v6 90%(확정: 깊이 +5mm 복원)**→v7 진행중(수평 hard gate 7mm — 초반 7/11 회귀 추세, **v7b 에서 hard gate 제거(점수 가중+descend_fix 트리거만 유지) 예정**). ⚠ **20ep ±2 는 PhysX 비결정성 잡음** — 같은 plan·spawn 도 run 간 성패 뒤집힘. 메커니즘 근거 없는 미세튜닝 금지.
...
- **다음 할 일**: ① v7 완료 분석 → v7b(h-gate soft화) 20ep 검증 ② v7b 영상 체인(1큐브 10ep `review_videos_v7/` + 4큐브 8ep `review_videos_4cube_v7/`) → 사용자 피드백 ③ 4큐브 신뢰성 측정 → 데이터셋 생성 게이트(LeRobot v3 — ⚠ SM 의 `CAMERA_KEYS`=top/wrist 2cam, North Star 3cam 계약과 불일치: 데이터셋 단계에서 front 추가 필요) ④ 남는 실패(reach 경계 spawn)는 scatter 안쪽/바깥 한계 1~2cm 축소(env 변경, 사용자 결정) 제안.
