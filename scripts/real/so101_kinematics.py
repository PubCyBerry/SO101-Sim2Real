"""SO-101 5축 닫힌 해(closed-form) FK/IK — Isaac 무의존 standalone.

`scripts/environments/pick_cube_state_machine.py:345-473` 의 `SO101Kinematics` 를 그대로
추출한 사본이다. 원본 파일은 모듈 상단에서 `from isaaclab.app import AppLauncher` 로 Isaac Sim 을
부팅하므로 실기기(Windows 네이티브) 스크립트에서 import 할 수 없다. 이 클래스는 `math` 만 쓰는
순수 Python 이라 좌표 상수·검증(FK err 1.5mm)이 sim 과 동일하게 유지된다.

좌표계: robot base_link frame. 단위: 관절각 radian, TCP 위치 meter.
"""
