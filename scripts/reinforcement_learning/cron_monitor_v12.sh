#!/usr/bin/env bash
# grasp_v12 LSTM+PPO 학습 자동 점검 (cron, 세션 독립).
# 30분 주기로 최신 체크포인트에 monitor_eval 을 돌려 scratch/full/pre 단계별
# 성공률을 집계하고, 16-env 그리드 추론 비디오를 녹화·보존한다.
#
# 등록 예 (crontab -e): */30 * * * * /…/cron_monitor_v12.sh >> /…/logs/cron_monitor_v12.cron.log 2>&1
#
...
RUN_NAME="lstm256_stage1_grasp_v12"                    # v12 run 고정
LOCK=/tmp/cron_monitor_v12.lock
