...
# --- 데모 기록 (RL demo-state reset 용) ---
parser.add_argument("--record_demos", default=None,
                    help="지정 시 매 step env-local scene state + 87dim obs + action + phase 를 "
                         "기록하고, 성공(모든 active 큐브 placed) 에피소드만 <dir>/demo_*.pt 로 저장")
parser.add_argument("--num_episodes", type=int, default=1,
                    help="기록 모드에서 reset→run 을 반복할 횟수 (에피소드당 num_envs 궤적). "
                         "총 데모 ≈ num_episodes × num_envs × 성공률")
parser.add_argument("--demo_tag", default="",
                    help="저장 파일명에 붙일 태그 (예: 'c1' 단일큐브). 미지정 시 active_objects 사용")
...
from sim_to_real.tasks.common.mdp.observations import rl_state as _rl_state  # noqa: E402
...
class SO101PickPlaceSM:
    ...
    def __init__(self, env, active_cubes: list[str]) -> None:
        ...
        # --- 데모 기록 버퍼 (RL demo-state reset 용) ---
        # 매 step env-local scene state(이전상태 s_t) + 87dim obs + action a_t + phase 를
        # CPU 로 누적. run() 종료 시 성공 env 의 궤적만 .pt 로 저장한다.
        self._rec = args.record_demos is not None
        self._base_env = env.unwrapped
        if self._rec:
            self._rec_jpos: list = []   # [T] each (N, ndof)
            self._rec_jvel: list = []
            self._rec_cpos: list = []   # [T] each (N, n_cube, 3) env-local
            self._rec_cquat: list = []  # [T] each (N, n_cube, 4)
            self._rec_cvel: list = []   # [T] each (N, n_cube, 6) lin+ang
            self._rec_bpos: list = []   # [T] each (N, 3) env-local
            self._rec_bquat: list = []  # [T] each (N, 4)
            self._rec_bvel: list = []   # [T] each (N, 6)
            self._rec_obs: list = []    # [T] each (N, 87)
            self._rec_act: list = []    # [T] each (N, 6)
            self._rec_phase: list = []  # [T] each list[str] len N
    ...
    def _act_all(self, q_list: list[list[float]], grip_targets: list[float]) -> None:
        ...
        if self._rec:
            self._capture_step(action)  # 상태 s_t + 적용 직전 action a_t 기록
        self.env.step(action)

    def _capture_step(self, action: torch.Tensor) -> None:
        """현재(step 직전) env-local scene state + 87dim obs + action + phase 를 CPU 버퍼에 누적."""
        origins = self.scene.env_origins  # (N, 3)
        self._rec_jpos.append(self.robot.data.joint_pos.detach().cpu().clone())
        self._rec_jvel.append(self.robot.data.joint_vel.detach().cpu().clone())
        cpos, cquat, cvel = [], [], []
        for c in CUBE_NAMES:
            a = self.scene[c]
            cpos.append((a.data.root_pos_w - origins).detach().cpu())            # (N,3) env-local
            cquat.append(a.data.root_quat_w.detach().cpu())                      # (N,4)
            cvel.append(torch.cat([a.data.root_lin_vel_w, a.data.root_ang_vel_w], dim=-1).detach().cpu())  # (N,6)
        self._rec_cpos.append(torch.stack(cpos, dim=1))                          # (N, n_cube, 3)
        self._rec_cquat.append(torch.stack(cquat, dim=1))                        # (N, n_cube, 4)
        self._rec_cvel.append(torch.stack(cvel, dim=1))                          # (N, n_cube, 6)
        bowl = self.scene[BOWL_NAME]
        self._rec_bpos.append((bowl.data.root_pos_w - origins).detach().cpu())   # (N,3)
        self._rec_bquat.append(bowl.data.root_quat_w.detach().cpu())             # (N,4)
        self._rec_bvel.append(
            torch.cat([bowl.data.root_lin_vel_w, bowl.data.root_ang_vel_w], dim=-1).detach().cpu())  # (N,6)
        obs = _rl_state(
            self._base_env, object_names=CUBE_NAMES, container_name=BOWL_NAME,
            include_velocities=True, include_orientation=True,
            include_container_orientation=True,
            object_half_extents=(0.015, 0.015, 0.020, 0.020),
            num_active=len(self.all_cubes))
        self._rec_obs.append(obs.detach().cpu())                                 # (N,87)
        self._rec_act.append(action.detach().cpu().clone())                      # (N,6)
        self._rec_phase.append([self.phase[e].name for e in range(self.num_envs)])
...
def _save_demos(sm: "SO101PickPlaceSM", out_dir: str, ep_idx: int, tag: str) -> int:
    """SM 의 step 버퍼에서 성공(모든 active 큐브 placed) env 궤적만 <out_dir>/demo_*.pt 로 저장.
    위치는 env-local (cube_pos/bowl_pos = root_pos_w - env_origin) — 학습 시 임의 env 로 주입 가능."""
    T = len(sm._rec_jpos)
    if T == 0:
        return 0
    jpos  = torch.stack(sm._rec_jpos, 0)   # (T, N, ndof)
    jvel  = torch.stack(sm._rec_jvel, 0)
    cpos  = torch.stack(sm._rec_cpos, 0)   # (T, N, ncube, 3)
    cquat = torch.stack(sm._rec_cquat, 0)
    cvel  = torch.stack(sm._rec_cvel, 0)
    bpos  = torch.stack(sm._rec_bpos, 0)   # (T, N, 3)
    bquat = torch.stack(sm._rec_bquat, 0)
    bvel  = torch.stack(sm._rec_bvel, 0)
    obs   = torch.stack(sm._rec_obs, 0)    # (T, N, 87)
    act   = torch.stack(sm._rec_act, 0)    # (T, N, 6)
    phases = sm._rec_phase                 # [T][N]
    n_active = len(sm.all_cubes)
    os.makedirs(out_dir, exist_ok=True)
    tg = tag or f"c{n_active}"
    saved = 0
    for e in range(sm.num_envs):
        if not all(sm._placed(c, e) for c in sm.all_cubes):
            continue
        traj = {
            "joint_pos":  jpos[:, e].contiguous(),
            "joint_vel":  jvel[:, e].contiguous(),
            "cube_pos":   cpos[:, e].contiguous(),    # (T, ncube, 3) env-local
            "cube_quat":  cquat[:, e].contiguous(),
            "cube_vel":   cvel[:, e].contiguous(),
            "bowl_pos":   bpos[:, e].contiguous(),    # (T, 3) env-local
            "bowl_quat":  bquat[:, e].contiguous(),
            "bowl_vel":   bvel[:, e].contiguous(),
            "obs":        obs[:, e].contiguous(),     # (T, 87)
            "actions":    act[:, e].contiguous(),     # (T, 6)
            "phases":     [phases[t][e] for t in range(T)],
            "meta": {
                "active_objects": n_active, "success": True, "frames": T,
                "object_radius_scale": args.object_radius_scale,
                "container_angle_scale": args.container_angle_scale,
                "episode": ep_idx, "env": e, "cube_names": list(CUBE_NAMES),
            },
        }
        torch.save(traj, os.path.join(out_dir, f"demo_{tg}_ep{ep_idx:03d}_env{e:03d}.pt"))
        saved += 1
    return saved
...
def main() -> None:
    ...
    # 데모 기록 시 학습 env 와 동일 커리큘럼 적용 — 비활성 큐브(z=-1) 비활성화 + obs num_active
    # 마스킹 정합. (안 하면 단일/멀티 큐브 데모에 비활성 큐브가 책상 위 distractor 로 남아
    # demo-reset 주입 시 학습 env 와 불일치.) container_radius_scale=1.0 고정(금지선).
    if args.record_demos is not None:
        from sim_to_real.tasks.pick_cube.pick_cube_env_cfg import apply_curriculum as _apply_cube_curr
        _apply_cube_curr(
    ...
    if args.record_demos is not None:
        ...
        for ep in range(args.num_episodes):
            ...
            sm.run()
            n = _save_demos(sm, args.record_demos, ep, args.demo_tag)
            total_saved += n
            log(f"[DEMO] episode {ep}: +{n} success trajectories (total {total_saved})")
        log(f"[DEMO] DONE: {total_saved} trajectories saved to {args.record_demos}")
        env.close()
        return
