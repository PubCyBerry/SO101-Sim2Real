"""SO-101 5축 닫힌 해 기구학 (standalone, ovphysx P1용).

URDF(assets/robots/urdf/so_arm101.urdf) origin 체인을 base frame 에서 전개한 결과.
이 모듈은 순수 Python이며 IsaacLab 또는 AppLauncher 의존성이 없다.

검증: --calibrate 를 통해 FK error 1.5mm 검증됨(pick_cube_state_machine.py 참조).
"""
...
class SO101Kinematics:
    ...
    def ik(
        ...
        for sign in (-1.0, 1.0):
            th1 = base_ang - math.atan2(
                self.L2 * math.sin(sign * rel),
