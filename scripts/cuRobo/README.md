...
- **pan 고정점 ×5**: tcp 목표 = `pad_target − R·FIXED_INNER_CENTER` 의 lateral offset 이 ρ 와 함께 돌아 pan 평면을 벗어남 → `pan ← atan2(tcp_tgt − PAN_AXIS_XY)` 재정렬.
...
- 게이트: `e_normal∈[2,8]mm` · `|e_tangent|≤E_TANGENT_MAX` · `|e_height|≤E_HEIGHT_MAX` · `|alpha(face_angle)|≤40°` · wrist_roll 물리범위. IK 성공만으론 불충분.
