...
python -m curobo.examples.getting_started.build_robot_model \
    ...
    --asset-path assets/robots/urdf \
    --output assets/robots/so101.yml \
    --clip-link base_link z 0.0 \
    --compute-metrics \
    --visualize
2. 이슈 존재하는 파츠 refit-link, sphere density로 피팅
python -m curobo.examples.getting_started.build_robot_model \
    --edit-config assets/robots/so101.yml \
    --output assets/robots/so101_refit.yml \
    --refit-link moving_jaw_so101_v1_link \
    --clip-link base_link z 0.0 \
    --sphere-density 6 \
    --visualize
"""
