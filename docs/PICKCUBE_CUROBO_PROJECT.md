...
| 2026-06-18 | **4-cube 1024 등량학습 3모델 + closed-loop eval 🟢** | 교란변수 통제 비교: 현재 DR 4-cube **1024ep**(504515 frame)→HF→**등량 640k samples** 학습 — ACT(b32×20k)·SmolVLA(b32×20k)·GR00T-N1.7(b8×80k). HF `taehunkim/so101_{sim_pick_cube_4cube_1024, act_*, smolvla_*, groot_n17_*}`. **closed-loop eval seed0 N=10 4-cube**: 셋 다 **all-4 완주 0%**(drift 벽), per-cube GR00T **2.5%**(1개 안착)>ACT/SmolVLA 0%, ever-in-bowl **SmolVLA 20%>GR00T 12.5%>ACT 5%**(SmolVLA 관여 최고·GR00T 유일 완성). open-loop 정상이나 closed-loop drift 미해결 → 다음=recovery 궤적·VLA-RFT. 스크립트 `run_4cube_1024_{pipeline,eval}.sh`·`demo_vla.sh`. 함정: `-e POLICY_PROFILE` compose 보간 안 됨(셸-env 프리픽스)·vla node `_load_env` override(임시 `*_4ceval.env`+`-e POLICY_PROFILE`)·gen Isaac teardown 좀비·hf upload 401(`--token`) |
