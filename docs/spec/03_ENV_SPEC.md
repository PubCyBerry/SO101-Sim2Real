...
> extrinsic DR(§11.7)이 이 prim local 값을 nominal 로 읽는 이유다.
...
| `reset_camera_extrinsic_dr` | `reset` | 카메라 pose DR **상태 초기화 + episode bias 재추첨**. frame-wise 갱신은 event 가 아니라 step 훅 — §11.7 |
...
### 11.7 카메라 extrinsic DR (6-DoF pose)
