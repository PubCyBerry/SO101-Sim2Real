...
# ===========================================================================
# 레퍼런스(ref_repos/pick_and_place) 보상 정합 — IsaacLab Lift-Cube-Place 레시피
# ---------------------------------------------------------------------------
# 성공이 확인된 레퍼런스의 6개 보상항을 SO-101+그릇 환경에 그대로 이식한다.
# 레퍼런스의 평면 마커 target_region → 우리 그릇(BOWL)으로 매핑하고, 높이는
# DESK_TOP_Z(책상면) 기준으로 변환한다(레퍼런스는 책상=z0 절대좌표).
# 레퍼런스가 단일 객체 레시피이므로 active 큐브에 대해 per-cube 합산한다
# (active_objects=1 이면 레퍼런스와 수치적으로 동일). action_rate/joint_vel 페널티는
# 우리 base RewardsCfg 의 mdp.action_rate_l2/joint_vel_l2 를 그대로 쓴다(레퍼런스와 동일 함수).
# ===========================================================================
...
def object_lowering_ref(
    ...
    ``env._ref_prev_cube_z`` (num_envs, K) 버퍼에 보관·매 step 갱신. reset 직후 상승 점프는
    clamp(min=0) 으로 흡수(레퍼런스 prev_cube_height 와 동일 동작). active 큐브 합산. weight 7.0.
    """
    cfgs = _make_object_cfgs(object_cfgs)
    target = _container_pos_local(env, container_center_xy, container_cfg)
    cur_z = torch.stack(
